...
class PhononBandPlotter:
    ...
    def plot_bands(self, material, bands, k_values, num_kvalues, 
        ...
        if kpoint_labels and len(kpoint_labels) == len(k_values):
            # Check if labels are meaningful (not just K0, K1, K2... format)
            has_meaningful_labels = any(
                not (label.startswith('K') and len(label) > 1 and label[1:].isdigit())
            ...
            if has_meaningful_labels:
                ...
                print(f"Using labels: {kpoint_labels}")
            else:
                # Labels are just K0, K1, K2... so use defaults for now
            ...
            # No labels or wrong number of labels, use defaults for now
            default_labels = [f'K{i}' for i in range(len(k_values))]
            ax1.set_xticklabels(default_labels)
...
def main():
    ...
    plotter.process_all_materials(figsize=(5, 4), save_format='pdf')

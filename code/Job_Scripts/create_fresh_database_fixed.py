...
1. AttributeError with d12_dir 
2. Incorrect D3 settings assumptions
3. Missing k_path_labels extraction
4. Full compatibility with run_workflow.py
...
class WorkflowDatabaseCreator:
    ...
    def create_fresh_database(self):
        ...
        with self.db._get_connection() as conn:
            ...
            for column_def in [
                ('calculations', 'input_settings_json', 'TEXT'),
                ('calculations', 'last_updated', 'TEXT'),
                ('calculations', 'job_status', 'TEXT DEFAULT "unknown"'),
                ('files', 'metadata', 'TEXT')
            ]:
                try:
                    table, column, definition = column_def
                    conn.execute(f'ALTER TABLE {table} ADD COLUMN {column} {definition}')
                    print(f"   ✅ Added {table}.{column} column")
                except Exception:
                    pass  # Column already exists
                    
            # Check if materials table has metadata column
            try:
    ...
    def _extract_input_settings(self, input_file: Path, calc_id: str):
        """Extract input settings from D12/D3 file with proper D3 handling."""
        try:
            # Get material_id for D3 inheritance linking
            material_id = None
            with self.db._get_connection() as conn:
            ...
            # For D3 files, use specialized extraction that doesn't assume functional info
            if input_file.suffix == '.d3':
                settings = self._extract_d3_settings(input_file)
                if settings:
                    with self.db._get_connection() as conn:
                        conn.execute(
                    ...
                    print(f"      ⚙️  Extracted D3 settings from {input_file.name}")
                    
                    # For D3 files, try to link to corresponding SP calculation settings
                    if settings.get('settings_inherited') and material_id:
                        self._link_inherited_settings(calc_id, material_id, settings)
                else:
                    print(f"      ⚠️  Failed to extract D3 settings from {input_file.name}")
            else:
                # For D12 files, use enhanced extractor first
                try:
                    from input_settings_extractor import extract_and_store_input_settings
                    ...
                    if success:
                        ...
                        # Fallback to basic for D12 if enhanced fails
    ...
    def _extract_d3_settings(self, input_file: Path) -> Optional[Dict]:
        """Extract settings from D3 files with proper k-path handling."""
        try:
            ...
            settings = {
                ...
                'extraction_method': 'specialized_d3_extraction',
                'extracted_at': datetime.now().isoformat(),
                'calculation_type': 'properties',
                'file_source': 'property_calculation'
            }
            
            # Parse D3 file content for property-specific information
            ...
            if len(lines) > 0 and lines[0].strip() == 'BAND' and len(lines) > 3:
                k_path_labels = []
                k_path_segments = []
                
                print(f"        🛤️  Parsing k-path from BAND d3 file...")
                
                # Parse k-path from D3 file
                for i, line in enumerate(lines):
                    if i < 3:  # Skip header lines (BAND, file.out, parameters)
                        continue
                    if line.strip() == 'END':
                        break
                    
                    # Parse k-point pairs (e.g., "X G", "G L")
                    if ' ' in line.strip() and len(line.strip().split()) == 2:
                        k_points = line.strip().split()
                        k_path_labels.extend(k_points)
                        k_path_segments.append(f"{k_points[0]} -> {k_points[1]}")
                
                if k_path_labels:
                    # Store unique k-point labels
                    unique_labels = []
                    for label in k_path_labels:
                        if label not in unique_labels:
                            unique_labels.append(label)
                    
                    settings['k_path_labels'] = unique_labels
                    settings['k_path_segments'] = k_path_segments
                    settings['k_path'] = ' | '.join(k_path_segments)
                    settings['num_k_path_points'] = len(unique_labels)
                    
                    print(f"        🛤️  Extracted k-path: {settings['k_path']}")
            
            # For D3 files, explicitly mark that computational settings are inherited
            settings['settings_inherited'] = True
            settings['computational_method'] = 'inherited_from_sp'
            settings['note'] = 'Computational settings (functional, basis set) inherited from previous SP calculation'
            
            # Do NOT assume any functional information for D3 files
            settings['functional_info'] = 'inherited'
            settings['method_info'] = 'inherited'
            
            return settings
            
        except Exception as e:
            print(f"        ❌ Error extracting D3 settings: {e}")
            return None
    
    def _extract_basic_settings(self, input_file: Path) -> Optional[Dict]:
        """Basic settings extraction for D12 files."""
        try:
            with open(input_file, 'r') as f:
                content = f.read()
            ...
            # Only attempt functional identification for D12 files with clear indicators
            if 'DFT' in content:
                settings['method'] = 'DFT'
                # Look for common functionals (only if clearly specified)
    ...
    def _link_inherited_settings(self, calc_id: str, material_id: str, d3_settings: Dict):
        """Link D3 calculation settings to corresponding SP calculation."""
        try:
            with self.db._get_connection() as conn:
                # Find the corresponding SP calculation for this material
                cursor = conn.execute("""
                    SELECT calc_id, input_settings_json FROM calculations 
                    WHERE material_id = ? AND calc_type = 'SP' 
                    AND input_settings_json IS NOT NULL
                    ORDER BY created_at DESC LIMIT 1
                """, (material_id,))
                
                sp_result = cursor.fetchone()
                if sp_result:
                    sp_calc_id, sp_settings_json = sp_result
                    if sp_settings_json:
                        try:
                            sp_settings = json.loads(sp_settings_json)
                            
                            # Merge SP settings with D3 settings (preserve D3 structure, add inheritance info)
                            inherited_settings = d3_settings.copy()
                            inherited_settings.update({
                                'inherited_from_calc_id': sp_calc_id,
                                'parent_method': sp_settings.get('method', sp_settings.get('functional_info', {}).get('method', 'unknown')),
                                'parent_functional': sp_settings.get('functional', sp_settings.get('functional_info', {}).get('functional', 'unknown')),
                                'parent_basis_set': sp_settings.get('basis_set', 'unknown'),
                                'inheritance_confirmed': True,
                                'inheritance_timestamp': datetime.now().isoformat()
                            })
                            
                            # Ensure functional_info remains as 'inherited' for D3 files
                            inherited_settings['functional_info'] = 'inherited'
                            inherited_settings['method_info'] = 'inherited'
                            
                            # Update the D3 calculation with inherited settings
                            conn.execute(
                                "UPDATE calculations SET input_settings_json = ? WHERE calc_id = ?",
                                (json.dumps(inherited_settings), calc_id)
                            )
                            
                            print(f"        🔗 Linked to SP calculation: {sp_calc_id}")
                            
                        except json.JSONDecodeError:
                            pass
                
        except Exception as e:
            print(f"        ⚠️  Could not link inherited settings: {e}")
    ...
    def _print_final_statistics(self):
        ...
        with self.db._get_connection() as conn:
            ...
            # Show DAT files specifically
            cursor = conn.execute("""
                SELECT COUNT(*) FROM files 
                WHERE file_type = 'data' OR file_name LIKE '%.DAT'
            """)
            dat_count = cursor.fetchone()[0]
            print(f"\\n💾 DAT Files Discovered: {dat_count}")
            
            # Show k-path extractions
            cursor = conn.execute("""
                SELECT COUNT(*) FROM calculations 
                WHERE input_settings_json LIKE '%k_path%'
            """)
            kpath_count = cursor.fetchone()[0]
            print(f"🛤️  K-path Extractions: {kpath_count}")

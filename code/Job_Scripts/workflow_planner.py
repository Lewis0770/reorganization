...
class WorkflowPlanner:
    ...
    def setup_cif_conversion(self, cif_files: List[Path]) -> Dict[str, Any]:
        ...
        if use_defaults:
            ...
            print("This will run the NewCifToD12.py configuration workflow...")
            
            # Create a temporary script to run NewCifToD12.py configuration
            temp_cif = cif_files[0]  # Use first CIF as template
            temp_config_script = self.create_temp_cif_config_script(temp_cif)
            
            print(f"Running configuration with sample file: {temp_cif.name}")
            result = subprocess.run([sys.executable, temp_config_script], 
                                 capture_output=True, text=True, input="n\ny\n")  # Don't save, but get config
            
            if result.returncode == 0:
                # Parse the configuration from the script output or create manually
    ...
    def create_temp_cif_config_script(self, sample_cif: Path) -> str:

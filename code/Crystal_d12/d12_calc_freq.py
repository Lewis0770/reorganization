...
def get_advanced_frequency_settings():
    ...
    if template_choice in template_map:
        ...
        if template_choice == "1":  # Basic template
            ...
            print("1: Automatic with coordinates (based on crystal symmetry)")
            print("2: Automatic with labels (based on crystal symmetry)")  
            print("3: Manual path definition")
            path_choice = input("Select path method (1-3) [2]: ").strip() or "2"
            
            if path_choice == "1":
                freq_settings["bands"]["path"] = "AUTO"
                print("\n✓ Will use automatic path with integer coordinates based on crystal symmetry")
            elif path_choice == "2":
                # Automatic with high-symmetry labels
                freq_settings["bands"]["shrink"] = 0  # Use labels mode
                freq_settings["bands"]["path"] = "AUTO"
                print("\n✓ Will use automatic path with high-symmetry labels (G, X, M, etc.) based on crystal symmetry")
            else:
                # Manual path definition
                print("\nManual path definition:")
                print("1: Use high-symmetry point labels")
                print("2: Use fractional coordinates")
                manual_choice = input("Select format (1-2) [1]: ").strip() or "1"
                
                if manual_choice == "1":
                    # Manual with labels
                    freq_settings["bands"]["shrink"] = 0  # Use labels mode
                    print("\nEnter high-symmetry path segments (e.g., G X, X M):")
                    print("Common labels: G (Gamma), X, M, K, L, W, U, R, A, Z, Y")
                    ...
                    for i in range(n_segments):
                        ...
                        if len(segment.split()) == 2:
                            ...
                            print(f"Invalid segment '{segment}', using 'G X'")
                            path.append("G X")
                    freq_settings["bands"]["path"] = path
                else:
                    # Manual with coordinates
                    n_segments = int(input("Number of path segments: "))
                    path = []
            ...
            # K-point density
            if path_choice != "2":  # Not using labels
            ...
            # Phonon DOS - ask about supercell
            print("\nPhonon DOS settings:")
            ...
            # DOS parameters
            custom_dos = yes_no_prompt("\nCustomize DOS parameters?", "no")
            if custom_dos:
            ...
            if calc_bands:
                ...
                if band_type == "2":
                    ...
                    if path_choice == "1":
                        ...
                        # Template selection - will be handled similarly to d3_config.py
                        print("\nTemplate selection not yet implemented")
                        print("Using automatic path instead")
                        ...
                        print("\nManual k-point path definition:")
                        n_segments = int(input("Number of path segments: "))
...
def get_auto_phonon_path(crystal_system: str = None, space_group: int = None, 
    ...
    Args:
        ...
        format_type: "labels", "vectors", or "literature" for comprehensive paths
    ...
    if format_type == "seekpath" and get_seekpath_full_kpath:
        coord_segments = get_seekpath_full_kpath(space_group, lattice_type)
        if coord_segments:
    ...
    if freq_settings.get("dispersion", False):
        ...
        if "bands" in freq_settings:
            ...
            if path == "AUTO":
                ...
                # Get lattice type (try to extract from crystal_system or default to P)
                ...
                # Generate path based on crystal system, space group, and format
                path = get_auto_phonon_path(crystal_system, space_group, shrink, 

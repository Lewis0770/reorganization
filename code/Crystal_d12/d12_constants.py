...
def configure_tolerances(shared_mode: bool = False, calculation_type: str = None) -> Dict[str, Any]:
    ...
    # Provide recommendations based on calculation type
    if calculation_type == "FREQ":
        print("\nRecommended tolerances for FREQ calculations:")
        print("  - Standard: TOLINTEG: 7 7 7 7 14, TOLDEE: 7")
        print("  - TIGHT (recommended for FREQ): TOLINTEG: 9 9 9 11 38, TOLDEE: 11")
        print("  - The tight tolerances ensure accurate force constants and frequencies")
        default_prompt = "Use tight tolerances for FREQ (TOLINTEG: 9 9 9 11 38, TOLDEE: 11)?"
        default_answer = "yes"
    elif calculation_type in ["SP", "OPT"]:
        print("\nRecommended tolerances:")
        print("  - Standard: TOLINTEG: 7 7 7 7 14, TOLDEE: 7")
        print("  - Tight (for high precision): TOLINTEG: 9 9 9 11 38, TOLDEE: 11")
        default_prompt = "Use standard tolerances (TOLINTEG: 7 7 7 7 14, TOLDEE: 7)?"
        default_answer = "yes"
    else:
        default_prompt = "Use default tolerances (TOLINTEG: 7 7 7 7 14, TOLDEE: 7)?"
        default_answer = "yes"
    
    use_default = yes_no_prompt(default_prompt, default_answer)
    
    if use_default:
        # Set the appropriate default/recommended values based on calculation type
        if calculation_type == "FREQ" and "tight" in default_prompt.lower():
        ...
        print("  - Default: 7 7 7 7 14")
        print("  - Tight: 8 8 8 8 16")
        print("  - Very tight: 9 9 9 9 18")
...
def configure_scf_settings(shared_mode: bool = False) -> Dict[str, Any]:
    ...
    # First ask about default SCF settings
    use_default_scf = yes_no_prompt(
        "Use default SCF settings? (MAXCYCLE=800, FMIXING=30%)",
        "yes"
    )
    ...
    if use_default_scf:
        ...
        scf_settings["method"] = "DIIS"  # Default SCF method
        return scf_settings
    ...
    maxcycle_input = input("Enter MAXCYCLE [800]: ").strip()
    if maxcycle_input:
        try:
    ...
    if not use_default_fmixing:
        fmixing_input = input("Enter FMIXING percentage [30]: ").strip()
        if fmixing_input:
            try:
...
def select_basis_set(elements: List[int], method: str = "DFT", 
    ...
    if basis_config["basis_set_type"] == "EXTERNAL":
        ...
        external_options = {
            "1": "./full.basis.doublezeta/",  # DZVP-REV2
            "2": "./full.basis.triplezeta/",  # TZVP-REV2
        }

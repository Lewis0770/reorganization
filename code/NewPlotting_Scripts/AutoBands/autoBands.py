...
def     ipBANDS(material, E_l, E_u):
    ...
    # Merge labels for very close x-positions
    tolerance = 0.01  # Threshold for considering positions as "too close"
    merged_labels = []
    merged_x_labels = []
    skip_next = False
    ...
    for i in range(len(x_labels)):
        ...
        if i < len(x_labels) - 1:
            # Check if next position is too close
            distance = x_labels[i + 1] - x_labels[i]
            if distance < tolerance:
                # Merge the labels
                label1 = labels[i] if labels[i] else ""
                label2 = labels[i + 1] if labels[i + 1] else ""
                
                # Handle the merging based on what labels we have
                if label1 and label2:
                    # Both positions have labels - combine them
                    if '|' in label2:
                        # If second label is already combined (like Z|R2), 
                        # merge with the end part after the |
                        parts = label2.split('|')
                        merged_label = label1 + '|' + parts[-1]
                    else:
                        # Simple merge
                ...
                merged_x_labels.append(x_labels[i])
                skip_next = True
            else:
                # Normal distance - keep as is
    ...
    # Update labels and x_labels with merged versions
    labels = merged_labels
    x_labels = merged_x_labels
    ...
    fig.savefig(FIGDIR + material + '.BANDS.pdf', format='pdf', dpi=300)

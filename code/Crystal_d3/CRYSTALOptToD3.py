...
class D3Generator:
    ...
    def __init__(self, input_file: str, calc_type: str):
        ...
        self.output_dir = self.input_dir  # Same directory by default
    ...
    def _write_band_d3(self, config: Dict[str, Any]) -> str:
        ...
        if config.get("path_method") == "labels":
            ...
            space_group = self.structure_info.get('space_group', 1)
            lattice_type = self.structure_info.get('lattice_type', 'P')
            seekpath_labels = get_seekpath_labels(space_group, lattice_type)
        ...
        # Always include material name and path in title
        title = f"{self.base_name} - Band Structure{path_info}"
        if config.get("title") and config["title"] != "Band Structure":
            # If there's a custom title, append it
            title = f"{self.base_name} - {config['title']}{path_info}"
        lines.append(title)
        ...
        if config.get("path_method") == "labels":
            ...
            for i in range(n_segments):
                lines.append(f"{path[i]} {path[i+1]}")
    ...
    def _write_doss_d3(self, config: Dict[str, Any]) -> str:
        ...
        if d12_file.exists():
            # Use for_doss=True to get same values (not doubled)
            shrink_lines = extract_shrink_from_d12(str(d12_file), for_doss=True)
    ...
    def generate_d3(self, shared_config: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        if shared_config and self.calc_type != "CHARGE+POTENTIAL":
            ...
            if self.calc_type == "BAND" and config.get("auto_path", False):
                ...
                if config.get("path_method") == "labels":
                    ...
                    if config.get("seekpath_full", False):
                        ...
                        frac_segments = get_seekpath_full_kpath(space_group, lattice_type, str(self.input_file))
                        if frac_segments:
                            ...
                            config["path_labels"] = get_seekpath_labels(space_group, lattice_type)
        ...
        if self.calc_type == "BAND":
            ...
            print("\n=== Configuring combined CHARGE+POTENTIAL calculation ===")
            
            if shared_config:
                # Use shared config for both charge and potential
                charge_config = shared_config.get("charge_config", {})
                potential_config = shared_config.get("potential_config", {})
            else:
                # Configure charge density
                print("\n--- Charge Density Settings ---")
                charge_config = configure_d3_calculation("CHARGE", str(self.input_file))
                
                # Configure potential
                print("\n--- Electrostatic Potential Settings ---")
                potential_config = configure_d3_calculation("POTENTIAL", str(self.input_file))
    ...
    if args.batch:
        ...
        if args.config_file:
            ...
            if shared_config:
                ...
                return
        elif args.shared_settings and calc_type != "CHARGE+POTENTIAL":
        ...
        for out_file in out_files:
            ...
            generator = D3Generator(str(out_file), calc_type)
        ...
        generator = D3Generator(input_file, calc_type)

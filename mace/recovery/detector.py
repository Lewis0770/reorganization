...
from mace.recovery.pandas_utils import (
    get_pandas, pandas_available, read_csv, 
    create_dataframe, to_datetime
)
PANDAS_AVAILABLE = pandas_available()
pd = get_pandas() if PANDAS_AVAILABLE else None
...
class CrystalErrorDetector:
                ...
                if csv_path.exists():
                    try:
                        ...
                        if PANDAS_AVAILABLE:
                            ...
                            # Extract data_files column from list of dicts
                            results['categories'][category] = [row.get('data_files', '') for row in df if row.get('data_files')]
    ...
    def _calculate_error_trends(self, all_errors: List[Dict], report: Dict):
        ...
        if not all_errors or not PANDAS_AVAILABLE:
            return
            
        # Convert to DataFrame for easier analysis
        df = create_dataframe(all_errors)
        if PANDAS_AVAILABLE:
